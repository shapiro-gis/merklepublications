...
function buildLocationList(locationData) {
  ...
  locationData.features.forEach((location, i) => {
    ...
    //container.style.backgroundColor = '##ebe8e1';

    /* Add the link to the individual listing created above. */
    const link = container.appendChild(document.createElement('button'));
    link.className = 'title';
    link.id = 'link-' + prop.id;
    link.style.fontSize = "15px"; //Change font size of title
    link.style.alignSelf = "flex-start"; // Align the title with the top of the container
    link.style.marginLeft = '10px';
    link.style.letterSpacing = '0.5px'; // Add letter spacing

    const words = prop[columnHeaders[0]].split(' ');
    
    // Set the maximum word length per line
    const maxWordLength = 32;
    
    // Initialize a variable to store the line
    let line = '';
    
    // Loop through each word and add to the line
    words.forEach((word) => {
      if (line.length + word.length <= maxWordLength) {
        line += ' ' + word;
      } else {
        // Add the line to the link and start a new line
        link.innerHTML += '<p style="line-height: 2; margin: 1">' + line.trim() + '</p>';
        line = word;
      }
    });
    
    // Add the last line to the link
    link.innerHTML += '<p style="line-height: 2; margin: 1">' + line.trim() + '</p>';

    //link.innerHTML = '<p style="line-height: 2; margin: 1">' + prop[columnHeaders[0]] + '</p>';
    ...
    details.style.alignItems = 'left';
    details.style.marginLeft = '10px';
    details.style.marginRight = '15px';
   ...
   for (let i = 1; i < columnHeaders.length; i++) {
    if (i === 7) {
      continue; // Skip columnHeaders[7]
    }
    const div = document.createElement('div');
    const header = document.createElement('span');
    header.style.fontWeight = 'bold';
    header.style.fontSize = '13px';
    header.style.color = '#619b8a';
...
const animals = [
  ...
  { name: "Pronghorn", icon: "prong-resize-edit1.png" }
...
for (let i = 0; i < animals.length; i++) {
  ...
  icon.style.maxHeight = "50"; // set a maximum height for the icon image
  icon.style.maxWidth = "50"; // set a maximum width for the icon image
  const name = document.createElement("span");
  name.innerText = animal.name;
  name.style.textAlign = "center";
  name.style.fontSize = "13px"; // set the font size of the label to 16px
  name.style.color = '#E8E9EB';
